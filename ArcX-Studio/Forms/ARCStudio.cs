...
namespace ArcX_Studio
{
    public partial class ARCStudio : Form
    {
        ...
        {
            Messenger.MessageBox("========== ArcX Studio ==========\n========== Developed by DescZ ==========\n========== v1.5 (Release) - Build 04 ===========", "About", MessageBoxButtons.OK, MsgIcon.None);
        }
        ...
        public async void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                using (var ofd = new OpenFileDialog())
                {
                    ...
                    ofd.Filter = "ARC (Minecraft LCA)|*.arc";
                    ofd.Title = "Open a file - ArcX Studio LCE";
        ...
        {
            // Cleanes leftover files if crash or End task happenes
        ...
        private void SetStatus(string message)
        {
            if (message != null)
            {
                Text = message + " - ArcX Studio LCE"; // Set status based on the current file 
            }
            else
            {
                Text = "ArcX Studio LCE";
            }
        }
        ...
        private void EntryList_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                string selectedPath = EntryList.SelectedNode.Tag.ToString(); // Selected node from EntryList current path 
                ...
                if (ext == ".png") // No need to add other ext because LCE only uses pngs
                {
                    ...
                    imageViewer.InterpolationMode = InterpolationMode.NearestNeighbor; // DO NOT CHANGE UNLESS NOT MINECRAFT TEXTURES
                    ...
                    {
                        imageViewer.SetImageWithFade(skinPicture, 200);
                        DimLabel.Text = $"{skinPicture.Width} x {skinPicture.Height}  |"; // Selected image width and height
                    }
                    return;
                }

                else if (ext == ".fui" || ext == ".swf") 
                {
                    statusStrip1.Hide();
                    richTextBox1.Text = "";
                    richTextBox1.Hide();
                ...
                else if (Directory.Exists(selectedPath)) // Selected node is a folder - show nothing special
                {
                    richTextBox1.Hide();
                    imageViewer.Hide();
                    statusStrip1.Hide();
        ...
        private void EntryList_DoubleClick(object sender, EventArgs e)
        {
            if (EntryList.SelectedNode.Tag != null)
            {
                ...
                switch (Path.GetExtension(selected).ToLowerInvariant())
                {
                    ...
                    case (".swf"):
                        DialogResult dialog = Messenger.MessageBox("Not supported. do you want to extract it?", "Not Supported file type", MessageBoxButtons.YesNo, MessageBoxIcon.None);
                    ...
                    case (".col"):
                        DialogResult dialog1 = Messenger.MessageBox("Not supported. do you want to extract it?", "Not Supported file type", MessageBoxButtons.YesNo, MessageBoxIcon.None);
        ...
        #region Replace and delete and add file
        ...
        private void replaceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (ext == ".png")
                    {
                        imageViewer.Invalidate(); // Refresh PictureBox
                    }
                }
            }
        }
        ...
        private void addFile_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    string dest = Path.Combine(appdata, Path.GetFileName(file));
        ...
        {
            if (EntryList.SelectedNode == null)
            {
                MessageBox.Show("Select a file first");
                return;
            }
            string file = EntryList.SelectedNode.Tag as string;
            if (Messenger.MessageBox("Delete this file?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.None) == DialogResult.No)
                return;
            ...
            if (File.Exists(file))
            {
                ...
                {
                    File.Delete(file);
                    Messenger.MessageBox("File deleted!", "Success", MessageBoxButtons.OK, MessageBoxIcon.None);
                    EntryList.SelectedNode.Remove();
                }
                catch (Exception ex)
                {
